...
step "Gather system information"
...
stop_spinner ok
echo -e "     ${DIM}${DOC_FILE}${NC}"

step "Configure auto-start"
...
EOF
    ...
    # Install service
    start_spinner "Installing mealplanner.service"
    ...
    if detect_clusterhat; then
        echo -e "  ${BLUE}ℹ️  ClusterHAT detected — installing delayed Zero W deployment timer...${NC}"
...
EOF
        ...
        echo -e "  ${GREEN}✓${NC}  Zero W deployment timer installed (fires 2 min after boot)"
    fi
    ...
    echo ""
    if [[ ! $REPLY =~ ^[Nn]$ ]]; then
        start_spinner "Starting application"
    ...
    echo -e "  ${BLUE}ℹ️  Auto-start not configured.${NC} You can run this script again anytime to enable it."
fi

step "Install maintenance cron job"

start_spinner "Installing weekly image prune cron job"
...
stop_spinner ok
echo -e "     ${DIM}/etc/cron.weekly/podman-prune${NC}"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo -e "  ${BLUE}ℹ️  Skipped.${NC}"
fi

section "Summary" "🍽️"
echo -e "  ${BLUE}📄 Quick start guide:${NC}   ${GREEN}${DOC_FILE}${NC}"
echo -e "  ${BLUE}🌐 Access application:${NC}  ${GREEN}http://${IP_ADDRESS}:8080${NC}"
echo ""
